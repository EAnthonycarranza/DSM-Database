...
/**
 * UserPdfSigner.jsx - Fixed version with proper z-index and more signature options
 * - Fixed z-index issues to prevent button blocking
 * - Added more signature customization options
...
.layout {
  display: grid;
  grid-template-columns: minmax(0, 1fr) var(--right-panel); /* viewer | right panel */
  gap: var(--right-gap);
  padding: 16px;
...
/* no additional padding needed; panel sits beside viewer */
...
.layout .asideright {
  ...
  padding: 14px;
  box-shadow: var(--shadow);
  position: sticky; /* stay in view while scrolling */
  top: 88px;
  height: calc(100vh - 96px); /* align with viewer height */
  overflow: auto; /* scroll panel content */
  margin-left: var(--right-gap); /* ensure clear visual separation */
  grid-column: 2; /* explicitly place in right column */
}

/* Panel sits in grid, no fixed positioning needed */
...
const onSignatureImageUpload = async (e) => {
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        // compute base page sizes at scale=1 for consistent fitting
        const metas = [];
        ...
        setPagesMeta(metas);
        initialFitDoneRef.current = false; // allow auto-fit to run for this PDF
      } catch (err) {
  ...
  // Auto-fit to viewer width once when pages meta and viewer are available
  useEffect(() => {
    try {
      if (!pagesMeta || pagesMeta.length === 0) return;
      if (!viewerRef.current) return;
      if (initialFitDoneRef.current) return;
      const first = pagesMeta[0];
      const pageW = Number(first?.width || 0);
      const vw = Number(viewerRef.current.clientWidth || 0);
      if (!pageW || !vw) return;
      const inner = Math.max(100, vw - 36); // account for padding
  ...
  // Keep fit-to-width when the viewer column resizes (e.g., layout changes)
  useEffect(() => {
    if (!viewerRef.current) return;
    ...
    const ro = new ResizeObserver((entries) => {
      try {
        ...
        const pageW = Number(pagesMeta[0]?.width || 0);
        if (!pageW || !w) return;
        const inner = Math.max(100, w - 36);
        const target = Math.max(0.3, Math.min(3, +(inner / pageW).toFixed(2)));
        // only update if significantly different to avoid render loops
        if (Math.abs(target - scale) > 0.01) setScale(target);
      } catch {}
    });
  ...
  useEffect(() => {
    ...
    const drawPages = async () => {
      ...
      for (const meta of pagesMeta) {
        ...
        renderTasksRef.current[num] = task;
        try {
          await task.promise;
        } catch (err) {
          // Ignore cancellations from re-renders/scale changes
          if (!(err && err.name === 'RenderingCancelledException')) {
            console.warn('PDF render error:', err?.message || err);
          }
        }
      }
    };
    ...
    return () => {
      cancelled = true;
      // Best-effort cancel any ongoing renders
      try {
        const tasks = Object.values(renderTasksRef.current || {});
        tasks.forEach((t) => { try { t?.cancel?.(); } catch {} });
      } finally {
...
const adoptUploadedSignature = () => {
  ...
  // Helper: base64 (no data: prefix) -> Uint8Array
...
<button
  ...
  onClick={downloadPDF}
  disabled={!pdfBytes}
  title={!pdfBytes ? "Upload a PDF first" : undefined}
>
  <i className="fa-solid fa-download"></i> Download Signed
      ...
      <div className="layout" role="region" aria-label="PDF signer">
          ...
          <div className="pages">
            {!pdfBytes ? (
              <div style={{ 
                ...
                <p style={{ fontSize: '0.8rem' }}>Upload a PDF or load the demo to get started</p>
              </div>
            ) : (
              pagesMeta.map((meta) => {
                ...
                return (
                <div key={`page-${meta.num}`} className="page-wrap" style={{ width: w, height: h }}>
                  <canvas
        ...
        <aside className="asideright">
          ...
          <div className="fill-list">
            {fields.length === 0 ? (
              <div style={{ color: '#9fb0e8', fontSize: '0.95rem' }}>
                No fields yet. Load a template or open a PDF with predefined fields.
              </div>
            ) : (
              fields.map((field, i) => {
