...
function readBrand() {
  if (typeof document === "undefined") return { bar: "#7B1F2C", barAlt: "#C9A961", text: "#6F5E5E", grid: "#E8DDC8" };
  const cs = getComputedStyle(document.documentElement);
  const isDark = document.documentElement.getAttribute("data-theme") === "dark";
  return {
    bar: (cs.getPropertyValue("--primary").trim() || (isDark ? "#C9A961" : "#7B1F2C")),
    barAlt: (cs.getPropertyValue("--accent").trim() || (isDark ? "#9B3041" : "#C9A961")),
    text: (cs.getPropertyValue("--text-muted").trim() || (isDark ? "#B8A99A" : "#6F5E5E")),
    grid: (cs.getPropertyValue("--border").trim() || (isDark ? "#3A2632" : "#E8DDC8")),
  };
}
...
export function BarChart({ labels = [], values = [], width = 700, height = 220 }) {
  ...
  useEffect(() => {
    ...
    values.forEach((v, i) => {
      ...
      grad.addColorStop(0, brand.bar);
      grad.addColorStop(1, brand.barAlt);
      ctx.fillStyle = grad;
...
export function MultiLineChart({ labels = [], series = [], width = 700, height = 220 }) {
  ...
  useEffect(() => {
    ...
    series.forEach((s, si) => {
      ...
      ctx.strokeStyle = si % 2 ? brand.barAlt : brand.bar;
      ctx.stroke();
      ctx.fillStyle = brand.text;
...
export function FunnelChart({
  ...
  height = 260,
  showPercent = true,
  colors = ["#7B1F2C", "#C9A961"], // start -> end (DSM burgundy → gold)
  gap = 12,
  padding = 26,
}) {
