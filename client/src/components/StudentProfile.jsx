...
export default function StudentProfile() {
  const { api, data, params, setPage, goToStudent, setToast, setModal } = useApp();
                        ...
                        <td>
                          {d.url ? (
                            <button 
                              className="btn" 
                              onClick={() => {
                                if (d.kind === "photo" || /\.(png|jpe?g|gif|webp|svg)$/i.test(d.url || d.name || "")) {
                                  openImage(d);
                                } else {
                                  openPdf(d);
                                }
                              }}
                              style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: "none", color: "var(--primary)", padding: 0 }}
                            >
                              <FaEye /> {d.name || "Open Document"}
                            </button>
                          ) : (
                            d.name || "(unnamed)"
                          )}
                  ...
                  {photos.map((p) => (
                    ...
                    >
                      <div style={{ aspectRatio: "4/3", overflow: "hidden" }}>
                        <img src={p.url} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover", cursor: "pointer" }} onClick={() => openImage(p)} />
                      </div>
                      <div style={{ padding: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          <button className="btn small" onClick={() => openImage(p)}>
                            <FaEye /> Open
                          </button>
