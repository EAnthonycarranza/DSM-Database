...
const ADMIN_SESSION_MS = 3 * 60 * 60 * 1000; // 3 hours
...
useEffect(() => {
  ...
  const bootstrap = async () => {
    try {
      ...
      const token = LS.read(AUTH_LS_KEY, null)?.token;
      if (!token) {
        setReady(true);
        return;
      }

      // Parallelize store fetching
      const results = await Promise.all(
        STORES.map(async (s) => {
          if (s === "messages" || s === "notifications") return { store: s, data: [] };
          try {
            const headers = { 
              "Content-Type": "application/json",
              "Authorization": `Bearer ${token}`
            };
            const res = await fetch(`${apiBase}/${s}`, { 
              method: "GET", 
              credentials: "include",
              headers
            });
            ...
            return { store: s, data: Array.isArray(body) ? body : (s === "settings" ? body || defaultSettings : []) };
          } catch {
            return { store: s, data: s === "settings" ? defaultSettings : [] };
          }
        })
      );
      ...
      if (!cancelled) {
        const stores = {};
        results.forEach(r => { stores[r.store] = r.data; });
  ...
  const postJsonLoose = useCallback(async (path, bodyObj = {}) => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout
    ...
    try {
      ...
      clearTimeout(timeoutId);
      if (err.name === 'AbortError') {
        return { ok: false, status: 408, data: { error: "Request timed out. Please check your connection." } };
      }
      throw err;
    }
  }, [apiBase]);
  ...
  // Checks whether a stored session is still valid (admin = 3hr expiry)
  function isSessionValid(stored) {
    if (!stored?.authenticated || !stored?.user || !stored?.token) return false;
    const role = String(stored.user.role || "").toLowerCase();
    if (role === "admin" && stored.loginAt) {
      return Date.now() - stored.loginAt < ADMIN_SESSION_MS;
    }
    return true; // students / no loginAt → no forced expiry
  }
...
const [auth, setAuth] = useState(() => {
  ...
  try { window.localStorage.removeItem(AUTH_LS_KEY); } catch {}
  try { window.sessionStorage.removeItem(AUTH_LS_KEY); } catch {}
...
useEffect(() => {
  ...
  // --- Admin session expiry: auto-logout after 3 hours ---
  useEffect(() => {
    if (!auth?.authenticated || !auth?.user) return;
    const role = String(auth.user.role || "").toLowerCase();
    if (role !== "admin" || !auth.loginAt) return;

    const remaining = ADMIN_SESSION_MS - (Date.now() - auth.loginAt);
    if (remaining <= 0) {
  ...
  }, [auth?.authenticated, auth?.user?.role, auth?.loginAt]);
...
const setPresence = async (presence = "online") => {
  try {
    ...
    // Only skip if same status and within 90s cooldown
    ...
    // Inform server via Socket.IO
    if (socketRef.current?.connected) {
      try { socketRef.current.emit("presence:set", { presence }); } catch {}
    }
  } catch {}
};

  // Efficient Visibility & Activity Tracking
  useEffect(() => {
    ...
    const handleVisibility = () => {
      ...
      // Small delay before flipping to away to avoid churn on quick flips
      const delay = status === "away" ? 3000 : 0;
      debounceTimer = setTimeout(() => setPresence(status), delay);
    };
  ...
  const refreshStore = useCallback(async (store, { force = false } = {}) => {
    try {
      ...
      if (refreshingRef.current[store]) {
        return;
      }
...
// Optimized login: single request path, no auto-provisioning, proper error returns
const login = useCallback(async (identifier, password, { remember = false } = {}) => {
  ...
  // Send both email and username — server handles matching
  const isEmailLike = /@/.test(idRaw);
  const payload = {
    ...(isEmailLike ? { email: idRaw.toLowerCase() } : { username: idRaw }),
    password: pass,
  };
  ...
  // 2FA required — stash credentials for verifyMfa
  if (data?.need2FA) {
    setPending2FA({
  ...
  setPending2FA(null);
  const authData = { authenticated: true, user, token: data.token, loginAt: Date.now() };
  setAuth(authData);

  if (remember) {
    LS.write(AUTH_LS_KEY, authData);
  }
  try { window.sessionStorage.setItem(AUTH_LS_KEY, JSON.stringify(authData)); } catch (e) {}
  ...
  try {
    ...
    console.warn('AppContext: error in post-login tasks (non-fatal):', err);
  }

  return { success: true, user };
}, [postJsonLoose, setPresence, refreshStore]);

// Update logout to clear both storages, with role-aware default redirect
const logout = useCallback((redirect) => {
  // Prefer explicit redirect if it's a string, else choose based on current role
  const role = String(auth?.user?.role || "").toLowerCase();
  const target = (typeof redirect === 'string' ? redirect : null) ?? (role === "student" ? "/login" : "/admin/login");
  const id = auth?.user?.id;
...
// Verify 2FA — properly distinguishes TOTP (digits only) from backup codes (alphanumeric)
const verifyMfa = useCallback(async ({ code, backupCode, method } = {}) => {
  if (!pending2FA) {
  ...
  // Determine if this is a backup code or TOTP based on method hint or content
  const raw = String(code || backupCode || "").trim();
  if (!raw) {
    return { success: false, error: "Enter your 6‑digit code or a backup code." };
  }

  const isBackup = method === "backup" || backupCode || !/^\d{6}$/.test(raw);
  const payload = {
    ...(email ? { email } : { username }),
    password,
    // Send in the correct field so the server processes it properly
    ...(isBackup ? { backupCode: raw } : { otp: raw }),
  };
  ...
  if (res.ok && res.data?.success) {
    ...
    const authData = { authenticated: true, user, token: res.data.token, loginAt: Date.now() };
    setAuth(authData);
    setPending2FA(null);

    if (remember) LS.write(AUTH_LS_KEY, authData);
    try { window.sessionStorage.setItem(AUTH_LS_KEY, JSON.stringify(authData)); } catch {}
  ...
  const presenceFor = useCallback((u) => {
    ...
    // 1) Prioritize the signed-in user's local state (no network flicker)
    if (u.id === auth?.user?.id && auth?.authenticated) {
      return String(presenceRef.current.last || "online").toLowerCase();
    }

    // 2) Use explicit presence field if it's a known value
    const p = String(u.presence || "").toLowerCase();
    if (p === "away" || p === "offline") return p;
    if (p === "online") return "online";

    // 3) Fallback to lastSeen heuristic if no explicit presence
...
const api = useMemo(() => {
  return {
    ...
    async upload(files, { studentId, by } = {}) {
      // Ensure we have an array of files even if a FileList or single file is passed
      let arr = [];
    ...
    async start2FA(userId, label) {
      return await apiRequest(`/auth/2fa/start`, { method: "POST", body: JSON.stringify({ userId, label }) });
    },
    async verify2FA(userId, token) {
      return await apiRequest(`/auth/2fa/verify`, { method: "POST", body: JSON.stringify({ userId, token }) });
    },
    async disable2FA(userId) {
      return await apiRequest(`/auth/2fa/disable`, { method: "POST", body: JSON.stringify({ userId }) });
    },
    async resetPasswordRequest(email) {
      return await apiRequest(`/auth/password/reset`, { method: "POST", body: JSON.stringify({ email }) });
    },
  };
...
const engage = useMemo(() => {
  ...
  const value = useMemo(
    () => ({
      ...
      sessionExpiresAt: auth?.loginAt ? auth.loginAt + ADMIN_SESSION_MS : null,
