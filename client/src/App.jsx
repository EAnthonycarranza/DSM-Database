...
const AdminOnly = ({ children }) => {
  const { user, authenticated, sessionExpiresAt } = useApp();
  const location = useLocation();
  
  const isStudent = String(user?.role || "").toLowerCase() === "student";
  const isAdmin = String(user?.role || "").toLowerCase() === "admin";
  const sessionExpired = isAdmin && sessionExpiresAt && Date.now() >= sessionExpiresAt;

  if (authenticated && isStudent) return <Navigate to="/dashboard" replace />;
...
function AppLayout({ panels, setPanels, notifs }) {
  ...
  // Routes where the TopNav and sidebar should be hidden (Students/Login)
  ...
  const hideChrome = location.pathname === "/admin/login" || isStudentRoute;

  const { user, authenticated } = useApp();
  const isStudent = String(user?.role || "").toLowerCase() === "student";

  // Student users are forced to light theme
  useEffect(() => {
  ...
  return (
    <>
      ...
      <main className={hideChrome ? "full-screen" : "content"} id="content">
        <Routes>
          ...
          <Route path="/admin/pdf-editor" element={<AdminOnly><UnifiedPdfEditor /></AdminOnly>} />
          <Route path="/admin/template-studio" element={<AdminOnly><AdminPdfEditor /></AdminOnly>} />
      ...
      <Modal />
      <Toast />
      {!hideChrome && <QuickMessagePanel />}
    </>
  );
}

export default function App() {
  const { ready, panels, setPanels, setModal, api, authenticated } = useApp();
